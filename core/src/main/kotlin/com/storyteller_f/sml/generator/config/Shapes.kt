...
package com.storyteller_f.sml.generator.config
...
abstract class ShapeDrawable(private val shape: String) : Drawable() {
    ...
    override fun startTag(): String {
        val tint = buildString {
            if (tint?.tint != null) {
                appendLine("android:tint=\"${tint.tint}\"")
                if (tint.tintMode != null)
                    appendLine("android:tintMode=\"${tint.tintMode}\"")
            }
        }

        val inset = buildString {
            if (optionalInset?.left != null) appendLine("android:optionalInsetLeft=\"${optionalInset.left}\"")
            if (optionalInset?.top != null) appendLine("android:optionalInsetTop=\"${optionalInset.top}\"")
            if (optionalInset?.right != null) appendLine("android:optionalInsetRight=\"${optionalInset.right}\"")
            if (optionalInset?.bottom != null) appendLine("android:optionalInsetRight=\"${optionalInset.bottom}\"")
        }
        return buildString {
            appendLine(
                """
                |<shape android:shape="$shape"
                |    android:dither="$dither"
...
fun DrawableDomain.Rectangle(block: RectangleShapeDrawable.() -> Unit) {
    drawable = RectangleShapeDrawable().apply(block).output()
}

fun DrawableDomain.Oval(block: OvalShapeDrawable.() -> Unit) {
    drawable = OvalShapeDrawable().apply(block).output()
}
...
fun DrawableDomain.Ring(
    ...
    drawable = RingShapeDrawable(innerRadius, thickness, false).apply(block).output()
}
...
fun DrawableDomain.Line(block: LineShapeDrawable.() -> Unit) {
    drawable = LineShapeDrawable().apply(block).output()
}
